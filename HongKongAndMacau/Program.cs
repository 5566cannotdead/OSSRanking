...
namespace HongKongMacauPopularDevelopers
{
    ...
    public class Program
    {
        ...
        private static readonly string[] SearchQueries = {
            ...
            $"followers:>{MinFollowers}+location:\"Hong Kong, China\"",
            $"followers:>{MinFollowers}+location:\"HK\"",
            $"followers:>{MinFollowers}+location:\"Hongkong\"",
            $"followers:>{MinFollowers}+location:\"Central, Hong Kong\"",
            $"followers:>{MinFollowers}+location:\"Kowloon\"",
            $"followers:>{MinFollowers}+location:\"New Territories\"",
            $"followers:>{MinFollowers}+location:\"Tsim Sha Tsui\"",
            $"followers:>{MinFollowers}+location:\"Admiralty\"",
            $"followers:>{MinFollowers}+location:\"Wan Chai\"",
            $"followers:>{MinFollowers}+location:\"Causeway Bay\"",
            $"followers:>{MinFollowers}+location:\"Macau\"",
            $"followers:>{MinFollowers}+location:\"Macao\"",
            $"followers:>{MinFollowers}+location:\"Macau SAR\"",
            $"followers:>{MinFollowers}+location:\"Macao SAR\"",
            $"followers:>{MinFollowers}+location:\"Macau, China\"",
            $"followers:>{MinFollowers}+location:\"Macao, China\""
        };
        ...
        {
            Console.WriteLine("香港澳門知名GitHub用戶排名系統");
            ...
            Console.WriteLine("正在搜尋香港澳門地區的GitHub用戶...");

            // 清理並設定基本 GitHub API headers
            ClearHttpClientHeaders();
            httpClient.DefaultRequestHeaders.Add("User-Agent", "HK-Macau-Popular-Developers");
            ...
            Console.WriteLine($"找到 {allUsers.Count} 個香港澳門地區的GitHub用戶 (其中 {existingUsers.Count} 個已完成)");
            ...
            for (int i = 0; i < newUsersToProcess.Count; i++)
            {
                ...
                try
                {
                    ...
                    {
                        // 如果是組織用戶，從列表中移除
                        var indexToRemove = allUsers.FindIndex(u => u.Login == user.Login);
                        if (indexToRemove >= 0)
                        {
                            allUsers.RemoveAt(indexToRemove);
        ...
        static async Task<List<GitHubUser>> SearchGitHubUsers(string query)
        {
            ...
            while (page <= maxPages && hasUsersWith50PlusFollowers)
            {
                ...
                var itemsList = ((System.Collections.IEnumerable)items).Cast<dynamic>().ToList();
        ...
        static string GenerateMarkdown(List<GitHubUser> users)
        {
            ...
            sb.AppendLine("# 香港澳門知名GitHub用戶排名");
            sb.AppendLine();
            sb.AppendLine("## [好讀HTML版本](https://5566cannotdead.github.io/hk_macau_popular_developers)");
            sb.AppendLine();
            ...
            sb.AppendLine("> - 因為欄位有限，顯示只取前幾名專案，完整專案資料可以看 [資料集](https://github.com/5566cannotdead/hk_macau_popular_developers/blob/main/Users.json)");
            sb.AppendLine();
        ...
        static string GenerateHtml(List<GitHubUser> users)
        {
            ...
            sb.AppendLine("    <title>香港澳門知名GitHub用戶排名</title>");
            ...
            sb.AppendLine("</head>");
            sb.AppendLine("<body>");
            sb.AppendLine("<h1>香港澳門知名GitHub用戶排名</h1>");
